...
function MyOffers() {
  ...
  const getProducts = async (categoryId) => {
    ...
    const apiUrl = 'https://c98e-2603-8001-7502-80d8-6874-8f80-e75a-bea8.ngrok-free.app/service';
    

    const params = {
      channel: sessionStorage.getItem('bellChannel'),
      email: 'kirbyfike@gmail.com',
      categoryId: categoryId,
      offerEligibility: sessionStorage.getItem('offerElgibility')
    };
    ...
    for (const [key, value] of Object.entries(data.products.results)) {
      productList.push({
        product: value
      })
    }
            ...
            <div className="grid md:grid-cols-1 grid-cols-2 gap-6">
              {products.map((row, index) => (
                <div key={index} className="bg-white shadow rounded overflow-hidden group">
                  <Link to={`/do/${row.product.id}`}>
                    <div className="pt-4 pb-3 px-4">
                      <h3 className="text-xl font-semibold">{row.product.name["en-CA"]}</h3>
                    </div>
                  </Link>
                </div>
              ))}
