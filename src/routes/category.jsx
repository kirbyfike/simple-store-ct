...
function Category() {
  ...
  const getProducts = async (categoryId) => {
    ...
    const res1 =  await apiRoot.categories()
    .withKey({ key: categoryId})
    .get()
    .execute();

    let queryArgs = {
      ...setQueryArgs(),
      filter: `categories.id: "${res1.body.id}"`
    };
    ...
    let exisCust = sessionStorage.getItem('existingCust');
    let custType = sessionStorage.getItem('custType');
    let custLocation = sessionStorage.getItem('custLocation');
